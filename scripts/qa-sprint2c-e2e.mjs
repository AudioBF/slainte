/**
 * Build web isolado + Playwright para Sprint 2C (Hoje/Semana metas efetivas).
 *
 * Uso:
 *   node scripts/qa-sprint2c-e2e.mjs
 *   QA_SKIP_EXPORT=1 node scripts/qa-sprint2c-e2e.mjs
 *
 * Executa dois builds isolados: flag OFF e flag ON.
...
const artifacts = path.join(root, 'artifacts', 'qa-sprint-2c');
...
function baseEnv(dayTargetsFlag) {
  return {
    EXPO_NO_DOTENV: '1',
    ...
    EXPO_PUBLIC_USE_DAY_TARGETS: dayTargetsFlag,
  };
}

function exportIsolated(outDir, dayTargetsFlag) {
  const absOut = path.join(root, outDir);
  rmSync(absOut, { recursive: true, force: true });
  mkdirSync(artifacts, { recursive: true });

  const env = baseEnv(dayTargetsFlag);
  ...
  const wantsOn = dayTargetsFlag === 'true';
  const looksOn =
    /useDayTargets:\s*!0\b|useDayTargets:\s*true\b|parseDayTargetsFlag\)\("true"\)/.test(
      bundle,
    );
  if (wantsOn && !looksOn) {
    throw new Error(`${outDir}: esperado USE_DAY_TARGETS ON no bundle.`);
  }
  if (!wantsOn && looksOn) {
    throw new Error(`${outDir}: USE_DAY_TARGETS parece ON — deve permanecer OFF.`);
  }
  console.log(`[export ${outDir}] day-targets flag ${wantsOn ? 'ON' : 'OFF'} OK`);
}
...
function runPlaywright({ port, spec, dayTargetsOn }) {
  return new Promise((resolve, reject) => {
    waitForServer(port)
      .then(() => {
        const child = spawn('npx', ['playwright', 'test', spec], {
          ...
          env: {
            ...
            QA_ARTIFACTS: path.join('artifacts', 'qa-sprint-2c'),
            QA_DAY_TARGETS_ON: dayTargetsOn ? '1' : '0',
          },
          stdio: 'inherit',
          shell: true,
          windowsHide: true,
...
async function runSuite({ outDir, port, flag, spec, dayTargetsOn }) {
  const skipExport = process.env.QA_SKIP_EXPORT === '1';
  if (!skipExport) {
    exportIsolated(outDir, flag);
  } else {
  ...
  try {
    await runPlaywright({ port, spec, dayTargetsOn });
  } finally {
    stop();
  }
}
...
async function main() {
  console.log('=== Sprint 2C E2E QA (local, isolated, OFF + ON) ===');

  await runSuite({
    outDir: 'dist-qa-sprint2c-off',
    port: 4183,
    flag: '',
    spec: 'e2e/sprint2c-qa-off.spec.ts',
    dayTargetsOn: false,
  });

  await runSuite({
    outDir: 'dist-qa-sprint2c-on',
    port: 4184,
    flag: 'true',
    spec: 'e2e/sprint2c-qa-on.spec.ts',
    dayTargetsOn: true,
  });

  console.log('\nE2E Sprint 2C complete. Artifacts:', artifacts);
}
