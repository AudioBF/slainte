...
| `EXPO_PUBLIC_USE_EDGE_MEAL_PLAN` | `"true"` = Edge `generate-meal-plan`; `"false"` = client Gemini rollback (**production: `true` — canary ON**, recheck ⚠️ 2026-06-28; meal plan 5/5 spaced OK; ver `MEAL_PLAN_EDGE_ROLLOUT_RECHECK_V1.md`) |
...
- Meal plan **Lightweight v1** deployed (planned meals only; `recipes: []` on new generations). **Production uses Edge Meal Plan** (`EXPO_PUBLIC_USE_EDGE_MEAL_PLAN=true` — controlled canary ON since 2026-06-23); client Gemini fallback retained for rollback.
- Apple/Google OAuth.
