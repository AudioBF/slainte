...
| `EXPO_PUBLIC_USE_EDGE_MEAL_PLAN` | `"true"` = Edge `generate-meal-plan` (**production default — GO definitivo** ✅ 2026-06-28; ver `MEAL_PLAN_EDGE_ROLLOUT_FINAL_GO_V1.md`); `"false"` = client Gemini rollback |
...
**Production meal plan (2026-06-16):** **Meal Plan Lightweight v1** ✅ — weekly generation returns `plannedMeals` + `summary` only (`recipes: []`). Shopping list uses **`plannedMeals` whenever a plan exists** (Shopping Source Fix v1). **Shopping Quality v1** ✅ — richer plannedMeals prompt + server-side dedupe (~30–50 items). Edge smoke **3/3**, P95 **~33 s**. **Production Edge Meal Plan GO** (2026-06-28; canary 2026-06-23→2026-06-28): `EXPO_PUBLIC_USE_EDGE_MEAL_PLAN=true` on Vercel; client Gemini fallback retained. **Recipe on demand v1** ✅ — `generate-recipe` Edge + Dieta **Gerar receita** CTA. **Polish v1** ✅. **Recipe Quality v1** ✅ (2026-06-19). **Recipe UX Polish v1.1** ✅ — recipe screen copy deployed. **Small UX Sweep v1** ✅ (2026-06-19) — Dieta/Hoje/Compras copy polish; no Edge or schema changes. **Meal Plan Edge Canary Prep v1** ✅ (2026-06-16) — inspection + canary/GO-NO-GO doc only; prod stays client Gemini (`docs/private/MEAL_PLAN_EDGE_CANARY_PREP_V1.md`). **Meal Plan Edge Canary Execution v1** ⚠️ (2026-06-20) — Phase A GO, Phase B GO, Phase C PARTIAL/NO-GO (shopping UI 400); prod unchanged — not full canary PASS (`docs/private/MEAL_PLAN_EDGE_CANARY_RESULT.md`). **Meal Plan Edge Canary Phase C Fix v1** ✅ (2026-06-20) — client payload sanitization + store read fix; 5/5 preview plan+shopping UI; prod still client Gemini (`docs/private/MEAL_PLAN_EDGE_CANARY_PHASE_C_FIX_V1.md`). **Meal Plan Edge Preview Canary v1** ✅ (2026-06-22) — **GO**; cadeia funcional 5/5; gate estrito shopping 30–50: 4/5 (run 2 = 54 itens, funcionalmente OK); prod inalterada (`docs/private/MEAL_PLAN_EDGE_PREVIEW_CANARY_V1.md`). **Meal Plan Edge Production Canary v1** ✅ (2026-06-23) — **CANARY ON**; 3/3 core chain em produção; `EXPO_PUBLIC_USE_EDGE_MEAL_PLAN=true` no Vercel Production; bundle `useEdgeMealPlan:!0`; client fallback preservado (`docs/private/MEAL_PLAN_EDGE_PRODUCTION_CANARY_V1.md`). **Meal Plan Edge Production Stabilization v1** ⚠️ (2026-06-23) — **CANARY WARNING**; spot-checks 2/3 OK; run 3 `GEMINI_UNAVAILABLE` transitório; canary permanece ON (`docs/private/MEAL_PLAN_EDGE_PRODUCTION_STABILIZATION_V1.md`). **Meal Plan Edge Production Rollout Decision v1** ⚠️ (2026-06-23) — 5 spot-checks espaçados; meal plan 5/5 HTTP 200; decisão **CANARY WARNING** (`docs/private/MEAL_PLAN_EDGE_PRODUCTION_ROLLOUT_DECISION_V1.md`). **Shopping Production Spot-check v1** ✅ (2026-06-23) — **SHOPPING STABLE**; 4/4 Compras com request HTTP 200 (`docs/private/SHOPPING_PRODUCTION_SPOT_CHECK_V1.md`). **Meal Plan Edge Resilience v1** ✅ (2026-06-23) — meal-plan error copy + plan preserved UX; no auto-fallback (`docs/private/MEAL_PLAN_EDGE_RESILIENCE_V1.md`). **Meal Plan Edge Rollout Recheck v1** ⚠️ (2026-06-28) — pós-resiliência; 5 spot-checks espaçados; meal plan 5/5 HTTP 200; 3 PASS / 1 WARN / 1 FAIL (shopping automation flake); CANARY WARNING (`docs/private/MEAL_PLAN_EDGE_ROLLOUT_RECHECK_V1.md`). **Meal Plan Edge Rollout Final GO v1** ✅ (2026-06-28) — **GO definitivo** após recheck + confirmação manual Compras 3/3; flakes de shopping reclassificados; `EXPO_PUBLIC_USE_EDGE_MEAL_PLAN=true` permanece production default (`docs/private/MEAL_PLAN_EDGE_ROLLOUT_FINAL_GO_V1.md`). **App stability smoke** ✅ (2026-06-17). **Core Flow Release Checkpoint v1** ✅ (2026-06-18). See `docs/private/SMALL_UX_SWEEP_V1_RESULT.md`, `docs/private/RECIPE_UX_POLISH_V1_1_RESULT.md`, `docs/private/RECIPE_QUALITY_V1_RESULT.md`, `docs/private/CORE_FLOW_RELEASE_CHECKPOINT_RESULT.md`, and related result docs.
...
- Meal plan **Lightweight v1** deployed (planned meals only; `recipes: []` on new generations). **Production uses Edge Meal Plan** (`EXPO_PUBLIC_USE_EDGE_MEAL_PLAN=true` — **GO definitivo** since 2026-06-28); client Gemini fallback retained for rollback.
- Apple/Google OAuth.
