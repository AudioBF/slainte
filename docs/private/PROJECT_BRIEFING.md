...
**Production meal plan (2026-06-16):** **Meal Plan Lightweight v1** ✅ — weekly generation returns `plannedMeals` + `summary` only (`recipes: []`). Shopping list uses **`plannedMeals` whenever a plan exists** (Shopping Source Fix v1). **Shopping Quality v1** ✅ — richer plannedMeals prompt + server-side dedupe (~30–50 items). Edge smoke **3/3**, P95 **~33 s**. Production stays `EXPO_PUBLIC_USE_EDGE_MEAL_PLAN=false` (client Gemini). **Recipe on demand v1** ✅ — `generate-recipe` Edge + Dieta **Gerar receita** CTA. **Polish v1** ✅. **Recipe Quality v1** ✅ (2026-06-19) — `generate-recipe` prompt deployed; Dublin practicality, planned-meal fidelity, 4–7 steps, tighter macros; automated + manual 3-slot smoke PASS. **Recipe UX Polish v1.1** ✅ — recipe screen copy: servings context, card origin, macro/IA disclaimers; no schema or Edge changes. **App stability smoke** ✅ (2026-06-17). **Core Flow Release Checkpoint v1** ✅ (2026-06-18) — main production flow validated after Meal Plan Lightweight, Recipe On Demand, Shopping Source Fix, and Shopping Quality; PWA refresh and sign out/sign in passed with no loss of `plannedMeals`, `recipes`, `recipeId`, or `shopping`, and Hoje/Dieta/Compras remained coherent. See `docs/private/RECIPE_UX_POLISH_V1_1_RESULT.md`, `docs/private/RECIPE_QUALITY_V1_RESULT.md`, `docs/private/CORE_FLOW_RELEASE_CHECKPOINT_RESULT.md`, `docs/private/SHOPPING_QUALITY_V1_RESULT.md`, `docs/private/SHOPPING_SOURCE_FIX_RESULT.md`, `docs/private/APP_STABILITY_SMOKE_RESULT.md`, and meal-plan / recipe-on-demand result docs.
